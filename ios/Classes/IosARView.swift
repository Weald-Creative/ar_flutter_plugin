        ...
        return Future {promise in
            ...
            switch (dict_node["type"] as! Int) {
                ...
                case 1: // GLB Model from the web
                    ...
                    break
                case 2: // Add GLB Model from file storage on device
                    // Get path to downloaded asset on the file system
                    // Add object to scene
                    if let node: SCNNode = self.modelBuilder.makeNodeFromGltf(name: dict_node["name"] as! String, modelPath: dict_node["uri"] as! String, useFilePath: true, transformation: dict_node["transformation"] as? Array<NSNumber>) {
                        if let anchorName = dict_anchor?["name"] as? String, let anchorType = dict_anchor?["type"] as? Int {
                            switch anchorType{
                                case 0: //PlaneAnchor
                                    if let anchor = self.anchorCollection[anchorName]{
